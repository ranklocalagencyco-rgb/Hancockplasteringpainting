...
function areaFaqs(slug: string, name: string): QA[] {
  ...
  return [
    ...
    { q: `How much does plastering or painting cost in ${name}?`, a: `Every job in ${name} is quoted at a fixed price agreed before we start, with no hourly rates and no surprise add-ons. Call ${PHONE} for a free, no-obligation quote.` },
...
export function getFaqs(pathname: string): FaqResult | null {
  ...
  if (area && AREAS[area[1]]) {
    return {
      heading: `Plastering & painting FAQs for ${AREAS[area[1]]}`,
